...
$(document).ready(function(e) {
    $('.module').each( function(index, element) {
		...
		if ($(this).attr('id'))
		{
			...
			$.getJSON("/DocImage/js/supersizedModule.php", {
				"id": _id,
				"width":$('#'+_id).width(),
				"height":$('#'+_id).height()
			})
				.error(function(request,error) { alert("Erreur JSON : " + error); })
				.complete(function(data) {
					//slides = eval(data.responseText);
					// Déclenchement de supersized
					var thisHeight = $('#'+_id).height();
					var datas = $.parseJSON(data.responseText);
					
					$('#'+_id).css('height', thisHeight + 'px');
					$('#'+_id).css('width', '100%');
					$('#'+_id).css('overflow','hidden');
					
					for (var i=0; i < datas.length; i++) {
						$('#'+_id).append('<div class="banniere" style="background-repeat:no-repeat; background-image:url('+datas[i].image+'); width:100%; height:220px; padding: 5px"><div class="titre">' + datas[i].titre + '</div><div class="description">'+datas[i].description+'</div></div');	
					}
